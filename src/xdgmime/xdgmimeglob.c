			       ...
			       const char  *file_name)
{
  ...
  /* Add some missing but frequently seen globs */
  glob_hash->simple_node = _xdg_glob_hash_insert_text (glob_hash->simple_node, ".dll", strdup ("application/x-ms-dos-executable"));
  glob_hash->simple_node = _xdg_glob_hash_insert_text (glob_hash->simple_node, ".ini", strdup (XDG_MIME_TYPE_PLAIN_TEXT));
}
