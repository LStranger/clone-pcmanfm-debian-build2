...
gboolean is_theme_compatible_with_gnome( const char* theme_name ) {
  ...
  char* dir_icon_path;
  GDir* sub_dir;
  const char* icons[] = { "gnome-fs-directory.png",
...
gboolean show_preference_dialog( GtkWindow* parent )
{
  ...
  const char* terminal_programs[]={
    "gnome-terminal", "konsole", "rxvt", "xterm", "eterm", "mlterm"
  };
  ...
  if( appSettings.terminal ) {
    ...
    if( i >= G_N_ELEMENTS(terminal_programs) ) { /* Found */
  ...
  if( ret )
  {
    ...
    appSettings.iconTheme = gtk_combo_box_get_active_text( GTK_COMBO_BOX(icon_theme) );
    if( appSettings.iconTheme && !*appSettings.iconTheme) {
